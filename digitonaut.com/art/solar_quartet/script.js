...
function go() {
	...
	frame += .05;
       ...
       //  for(i=0;i<5;i++)for(X=w,c[fy]=`hsl(7,23%,${23-i*6}%`;X--;F[fc](X/4,U/4+1,1,w))c[fc](X,U=W+i*25+m((t+t*i*i)/1e3+X/2e3,i/19-.5)*45,1,w)
        ...
        for(let i = 0; i < 5; i++) {
            ...
            for(let x = canvas.width; x--;) {
                ...
                let y = 128 + i * 25 + mountainHeight(mountainPosition + i * 2, mountainRoughness) * 45;
